/*
 * A mip chain built by vkCmdBlitImage, then sampled.
 *
 * The same four flat-coloured levels, but the shader lets the sampler choose,
 * multiplying the coordinate so the texture is minified and a real derivative
 * has to be taken across the fragment quad. texturemipmapgen differs from
 * lavapipe in exactly this way -- not on edges, and systematically brighter --
 * so if the choice differs at all this reproduces it in a frame with four
 * known answers rather than a kaleidoscope.
 *
 * Explicit level of detail is cpvk_lod, and it already matches.
 *
...
main(int argc, char **argv)
{
   ...
   const char *fs_path = argc > 2 ? argv[2] : "/tmp/lat/lodimp.frag.spv";
   ...
   VkImageCreateInfo timgi = {
      ...
      .extent = { 8, 8, 1 }, .mipLevels = 4, .arrayLayers = 1,
   ...
   CHECK(vkBindImageMemory(dev, timg, tmem, 0));
   {
       ...
       * looks. */
      /*
       * Level 0 only, and a checkerboard rather than a flat colour: a flat
       * source downsamples to itself and would hide any filtering difference
       * entirely. The remaining three levels are left empty for
       * vkCmdBlitImage to generate, which is the one stage the native driver
       * and the Gallium one do not share -- this driver box-filters the source
       * footprint on the host, and the Gallium path goes through lavapipe.
       */
      ...
      for (unsigned y = 0; y < 8; y++)
         for (unsigned x = 0; x < 8; x++) {
            ...
            unsigned char v = ((x + y) & 1) ? 230 : 25;
            px[0] = v;
            px[1] = (unsigned char)(255 - v);
            px[2] = (unsigned char)(x * 32);
            px[3] = 255;
         }
   }
   ...
   /* Build levels 1..3 from level 0, halving each time, exactly as a mip
    * generator does and exactly as texturemipmapgen does. */
   for (unsigned l = 1; l < 4; l++) {
      /* The 1.0 entry point on purpose: this driver advertises apiVersion
       * 1.0, so vkCmdBlitImage2 is not dispatched for its device and calling
       * it jumps to a null pointer. */
      VkImageBlit region = {
         .srcSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, l - 1, 0, 1 },
         .srcOffsets = { { 0, 0, 0 }, { (int)(8u >> (l - 1)), (int)(8u >> (l - 1)), 1 } },
         .dstSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, l, 0, 1 },
         .dstOffsets = { { 0, 0, 0 }, { (int)(8u >> l), (int)(8u >> l), 1 } },
      };
      vkCmdBlitImage(cmd, timg, VK_IMAGE_LAYOUT_GENERAL,
                     timg, VK_IMAGE_LAYOUT_GENERAL, 1, &region,
                     VK_FILTER_LINEAR);
   }
