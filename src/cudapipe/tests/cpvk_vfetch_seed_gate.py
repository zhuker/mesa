...
This is the negative control that makes that visible. CUDAPIPE_VFETCH_SKIP_SEED
...
def main():
    ...
    good = run(exe, {"CUDAPIPE_FUSED_VFETCH": "1"})
    print("fused, seeded:      exit %d" % good.returncode)
    starved = run(exe, {"CUDAPIPE_FUSED_VFETCH": "1",
                        "CUDAPIPE_VFETCH_SKIP_SEED": "1"})
