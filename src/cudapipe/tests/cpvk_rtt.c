/*
 * The native driver's ninth test: render to a texture, then sample it.
 *
 * bloom renders its glow into a 256x256 target, blurs it by sampling that
 * target, and adds the result. Its blend state is correct and its offscreen
 * draws run, and the glow is entirely absent -- so the suspicion is that a
 * sampled image which was itself a colour attachment reads as nothing. No
 * other test here writes a texture with the rasterizer and then reads it.
 *
...
main(int argc, char **argv)
{
   ...
   const char *out = argc > 3 ? argv[3] : "/tmp/lat/native_rtt.ppm";
   ...
   /*
    * First pass: clear the texture to a colour the CPU never wrote there, by
    * rendering into it. If a sampled image that was a colour attachment reads
    * as its uploaded contents rather than what was rendered, this shows it.
    */
   VkRenderingAttachmentInfo tex_at = {
      .sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
      .imageView = tview,
      .imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
      .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
      .storeOp = VK_ATTACHMENT_STORE_OP_STORE,
      .clearValue.color.float32 = { 0.0f, 0.85f, 0.35f, 1.0f } };
   VkRenderingInfo tex_ri = {
      .sType = VK_STRUCTURE_TYPE_RENDERING_INFO,
      .renderArea = { { 0, 0 }, { 4, 4 } }, .layerCount = 1,
      .colorAttachmentCount = 1, .pColorAttachments = &tex_at };
   beginRendering(cmd, &tex_ri);
   endRendering(cmd);
