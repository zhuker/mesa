/*
 * A cube sampled with a direction that varies per fragment.
 *
...
main(int argc, char **argv)
{
   ...
   VkImageCreateInfo timgi = {
      ...
      .extent = { 8, 8, 1 }, .mipLevels = 4, .arrayLayers = 6,
         ...
         for (unsigned l = 0; l < 4; l++) {
            ...
            for (unsigned y = 0; y < dim; y++)
               for (unsigned x = 0; x < dim; x++) {
                  ...
                  px[0] = (unsigned char)(20 + f * 40);
                  px[1] = (unsigned char)(20 + l * 60);
                  px[2] = 128;
                  px[3] = 255;
               }
         }
   ...
   VkImageViewCreateInfo tvci = {
      ...
      .subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 4, 0, 6 } };
   VkImageView tview;
   CHECK(vkCreateImageView(dev, &tvci, NULL, &tview));

   VkSamplerCreateInfo sci = {
      .sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO,
      /* NEAREST: with no bilinear footprint there is nothing to blend across
       * a cube seam, so a driver that filters seamlessly and one that clamps
       * inside the face must agree exactly. */
