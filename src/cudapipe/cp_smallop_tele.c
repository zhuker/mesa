...
cp_smallop_report(void)
{
   ...
   fprintf(stderr, "cudapipe: smallops: submits=%llu frames=%.1f "
   ...
   for (unsigned k = 0; k < CP_SMALLOP_KINDS; k++) {
      if (!kind_count[k])
         continue;
      fprintf(stderr, "cudapipe: smallops %-12s count=%llu (<256B=%llu "
   ...
   for (unsigned i = 0; i < n; i++) {
      ...
      if (!c)
         continue;
      fprintf(stderr, "cudapipe: smallop site %-12s %s:%d",
