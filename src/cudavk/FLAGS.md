...
| `CUDAVK_PDL` | bool (value) | `off` | — | let a dependent kernel launch before its predecessor in the same stream has drained, by compiling griddepcontrol.wait into the dependent and launching it with the programmatic stream serialization attribute; needs compute capability 9.0 and CUDA 11.8, and falls back to a normal launch whenever the predecessor is not the expected kernel |
