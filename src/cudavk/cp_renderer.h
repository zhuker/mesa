 ...
 *
 * CUDAVK_DEBUG_BATCHDIFF names the fields two draws disagree on, which is
...
struct cp_context {
   ...
   /*
    * CUDAVK_TILE_CENSUS. The accumulation runs for a whole framebuffer
...
#define CP_FLUSH_GENS 8
    ...
    * CUDAVK_SPEC_STATS. The specialiser recognises an exact IR shape, so a
    ...
    * the drain it replaced). flush_gens is 1 under CUDAVK_FLUSH_DRAIN,
...
bool cp_abuf_shade(struct cp_context *cp,
                   ...
                   CUdeviceptr prim_refs, unsigned w, unsigned h,
...
void cp_shade_fragments(struct cp_context *cp,
                        ...
                        CUdeviceptr positions, CUdeviceptr vs_output_buf,
                        CUdeviceptr prim_refs, unsigned num_triangles,
                        unsigned w, unsigned h,
 ...
 * hence only under CUDAVK_DEBUG_TIME, and hence a pool rather than one pair
