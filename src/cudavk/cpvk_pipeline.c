...
cpvk_lower_nir(nir_shader *nir)
{
    ...
    * llvmpipe before cudavk ever sees the shader. Reaching the backend with
                          ...
                          VkPipelineLayout *pPipelineLayout)
{
   ...
   if (layout->vk.set_count + 1 > CP_MAX_CONST_BUFFERS)
      fprintf(stderr, "cudavk: pipeline layout has %u descriptor sets and "
              "there are %u slots\n", layout->vk.set_count,
                             ...
                             VkPipeline *pPipelines)
{
   ...
   for (uint32_t i = 0; i < count; i++) {
      ...
      if (stencil_visible) {
         ...
         if (!said_stencil++)
            fprintf(stderr, "cudavk: stencil test/write state is not "
