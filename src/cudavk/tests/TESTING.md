...
cudavk is checked differentially. The Sascha Willems samples at `~/git/Vulkan`
render offscreen and reproducibly, so the same binary is run against the NVIDIA
ICD, against cudavk, and against lavapipe/llvmpipe, and the frames are
...
SAMPLES="$S" VALIDATION=0 OUT=build/compare/cuda \
  VK_ICD_FILENAMES=$M/cudavk/cudavk_devenv_icd.x86_64.json ./run_offscreen.sh
SAMPLES="$S" VALIDATION=0 OUT=build/compare/llvmpipe \
  VK_ICD_FILENAMES=$M/lavapipe/lvp_devenv_icd.x86_64.json ./run_offscreen.sh

cd build/compare
python3 ~/mesa/src/cudavk/tests/cp_gallery.py ref cuda llvmpipe \
...
$T/cp_perf_run.sh cudavk $M/cudavk/cudavk_devenv_icd.x86_64.json $R/cuda     60
...
$T/cp_perf_run.sh cudavk $M/cudavk/cudavk_devenv_icd.x86_64.json $B/cuda     600 1
...
minutes for cudavk's pass at 600 frames.
...
them. It is the honest number for cudavk, which blocks the host on every draw,
...
none of cudavk's — do not read a `wall_s` total as rendering time.
...
llvmpipe is the backend cudavk's sampler and clipper were ported from and
...
it the same anisotropic filtering difference cudavk has.

Run it as a third driver in both sweeps. A residual both software renderers
share is what software rasterization costs; a residual only cudavk has is a
defect. That distinction has settled several questions that the NVIDIA delta
alone made look like cudavk bugs — `gltfscenerendering` grows fourfold over an
...
comparison set without touching cudavk. Re-render llvmpipe and `cmp` it
...
handoff as the one sample cudavk beat llvmpipe on, 14.2 s against 16.0. Those
...
against cudavk's 135, a factor of fifty the other way. Any measure that
...
`computeshader` on cudavk reports a best of 1.6 ms and a worst of 191 against
...
`cuLaunchKernel`, and cudavk issues thousands of them a frame, so a traced
...
**every shader cudavk compiles is a CUDA kernel named `main`**, so the vertex
...
`CUDAVK_NVTX=1`, so the driver pushes an NVTX range around each draw and each
...
launch-bound frame and the wrong one for anything else; `CUDAVK_DEBUG_TIME`
...
so they are off unless `CUDAVK_NVTX` is set. `NVTX=0 cp_profile.sh ...` turns
...
replay, each a one-line pipe: `CUDAVK_DEBUG_BATCH` and
`CUDAVK_DEBUG_BATCHDIFF` through `sort | uniq -c` gave the exact
distribution of batch-break causes (which is what made the per-draw-table
work targeted), and `CUDAVK_DEBUG_PASSSEQ` with
...
this document argues for below. At frame 47 of `gltfscenerendering` cudavk
...
so that one is a cudavk defect.
