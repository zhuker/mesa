 ...
 * On cudavk it requires one hardware/direct fragment launch, no A-buffer
...
run_wrapped(const char *self, int fault)
{
   ...
   if (pid == 0) {
      ...
      if (fault)
         setenv("CUDAVK_TEXTURE_CACHE_FAIL_AFTER_FS_ENQUEUE", "1", 1);
      else
         unsetenv("CUDAVK_TEXTURE_CACHE_FAIL_AFTER_FS_ENQUEUE");
