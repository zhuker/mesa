/*
 * The native driver's seventh test: two draws that differ only in their
 * descriptor set.
 *
 * This is the case batching gets wrong. gltfscenerendering merges nine draws
 * whose per-draw binding rows are demonstrably correct -- both stages', in
 * the debug output -- and renders the wrong frame anyway. Here it is in two
 * draws: one pipeline, one vertex buffer, one index buffer, two index ranges,
 * and a different descriptor set bound before each. Each set tints its
 * triangle a different colour, so a batch that shades both from one row is a
 * picture with one colour in it instead of two.
 *
 * The original comment follows, since the scaffolding is the uniform test's.
 *
 * The native driver's third test: uniform buffers reaching both stages.
 *
 * The same two triangles, but the vertex shader displaces them by a vector
 * from one uniform buffer and the fragment shader tints them by a colour from
 * another. Two separate bindings so that a shader reading the wrong table
 * entry produces the wrong picture rather than a plausible one, and both
 * stages so that a table wired for only one of them is visible.
 */
...
{
   const char *vs_path = argc > 1 ? argv[1] : "/tmp/lat/ubo.vert.spv";
   const char *fs_path = argc > 2 ? argv[2] : "/tmp/lat/ubo.frag.spv";
   const char *out = argc > 3 ? argv[3] : "/tmp/lat/native_batch.ppm";
