...
# the rate it can queue frames at. It is the honest number for cudavk, which
...
#   M=$HOME/mesa/build-cudavk/src/gallium/targets
#   T=$HOME/mesa/src/cudavk/tests
...
#
#   $T/cp_perf_run.sh cudavk $M/cudavk/cudavk_devenv_icd.x86_64.json $R/bench_cuda 60 1
#
...
LIST=${LIST:-$HOME/mesa/src/cudavk/tests/headless_streamer_samples.txt}
cd "$VULKAN" || exit 1
BIN=build/bin
