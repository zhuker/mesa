#!/bin/bash
#
# Where a cudavk frame actually goes, per sample, repeatably.
#
...
#
# CUDAVK_DEBUG_TIME=1 in the environment additionally captures the driver's
...
if [ "${METRICS:-0}" = "1" ]; then
    ...
    # count: CUPTI charges every cuLaunchKernel, cudavk issues thousands a
...
CUDAVK_NVTX_ENV=()
[ "${NVTX:-1}" = "1" ] && CUDAVK_NVTX_ENV=(env CUDAVK_NVTX=1)
"${CUDAVK_NVTX_ENV[@]}" "$NSYS" profile \
