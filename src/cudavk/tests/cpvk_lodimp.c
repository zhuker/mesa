/*
 * Implicit level of detail: the level the sampler picks itself.
 *
...
main(int argc, char **argv)
{
   ...
   CHECK(vkBindImageMemory(dev, timg, tmem, 0));
   {
       ...
       * looks. */
      /*
       * Four levels, each a single flat colour, so that a sample names the
       * level it came from with no filtering ambiguity: level 0 is red, 1 is
       * green, 2 is blue, 3 is white.
       */
      static const unsigned char lvl[4][3] = {
         { 200, 20, 20 }, { 20, 200, 20 }, { 20, 20, 200 }, { 230, 230, 230 },
      };
      void *p;
      CHECK(vkMapMemory(dev, tmem, 0, VK_WHOLE_SIZE, 0, &p));
      for (unsigned l = 0; l < 4; l++) {
         VkImageSubresource sub = { VK_IMAGE_ASPECT_COLOR_BIT, l, 0 };
