 ...
 * survives as local memory.  That rejects cudavk's strict HW_INLINE gate but
 * is resource-isolated and admitted as HW_FUSED.  The wrapper enables texture
 * cache statistics and requires every native cudavk fragment launch to use
 * HW_FUSED.  Other Vulkan drivers ignore the two environment variables.
 *
 * Build:
 *   cc -std=c11 -O2 -Wall -Wextra -Werror \
 *      src/cudavk/tests/cpvk_texture_cache_ops.c -lvulkan \
