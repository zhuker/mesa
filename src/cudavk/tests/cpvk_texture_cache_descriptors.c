 ...
 * which cudavk's strict HW_INLINE admission gate rejects the binary, while
 * the resource-isolated HW_FUSED texture binary is safe.  The outer process
 * enables CUDAVK_TEXTURE_CACHE_STATS (the hardware path is the default), captures
 * stderr, and on a cudavk device requires modes inline=0 and fused=all hits.
 ...
 *      src/cudavk/tests/cpvk_texture_cache_descriptors.c \
 ...
 *      /tmp/cpvk_texture_cache_descriptors
 *   VK_ICD_FILENAMES=.../cudavk_devenv_icd.x86_64.json \
 *      /tmp/cpvk_texture_cache_descriptors
 *
...
assert_native_stats(const char *log)
{
   ...
   if (!device || strncmp(device + strlen("cpvk-test-device: "),
                          "cudavk (", strlen("cudavk (")))
      return 0;

   const char *hw = strstr(log, "cudavk: hardware texture: ");
   ...
   if (!hw || !fallbacks || !modes ||
       sscanf(hw, "cudavk: hardware texture: %llu/%llu fragment launches hit",
              &hits, &launches) != 2 ||
   ...
   const char *resources = strstr(log, "cudavk: texture cache resources: ");
   ...
   if (!resources || !array_stats ||
       sscanf(resources,
              "cudavk: texture cache resources: hits=%llu fallbacks=%llu "
...
run_wrapped(const char *self)
{
   ...
   if (pid == 0) {
      ...
      if (dup2(fds[1], STDERR_FILENO) < 0)
         _exit(126);
      close(fds[1]);
      unsetenv("CUDAVK_NO_TEXTURE_CACHE");
      setenv("CUDAVK_TEXTURE_CACHE_STATS", "1", 1);
