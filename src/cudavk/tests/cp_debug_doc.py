...
CUDAVK_ABUFFER_TIMING defaulted on for about a month after the code had
...
# The native driver's own registry. The Gallium-hosted driver keeps a separate
# copy of cp_debug.c with a different set of flags, and its copy of this script
# checks that one: a single checker pointed at one of the two silently leaves
# the other driver's switches undocumented, which is how the native driver came
# to have seven flags no generated table mentioned.
DRIVER = HERE.parent
