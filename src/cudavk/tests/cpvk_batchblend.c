/*
 * The native driver's eighth test: two draws whose descriptor sets name
 * different textures.
 *
 ...
 * 60-byte stride, which CUDAVK_DEBUG_DRAW showed is the last thing this
 * test did not have.
 */
struct vertex {
   float x, y, z;        /* 0  */
                    ...
                    void *hook_data)
{
   ...
   /*
    * Blending on, which is the whole point of this variant: a blended draw
    * goes through the A-buffer, where the order fragments composite in is
    * decided per pixel rather than by submission order, and the batch front
    * end refuses to merge blended draws until something shows that merge is
    * right. This is that something.
    */
   ...
   VkPipelineDepthStencilStateCreateInfo ds = {
      ...
      /* Depth test on, depth writes off: what a transparent pass does, and
       * what makes the A-buffer decide the order. */
