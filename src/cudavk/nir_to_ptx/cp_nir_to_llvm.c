...
{
   fprintf(stderr, "cudavk: inline FS unavailable: matching clang "
...
cp_vs_census_report(void)
{
   ...
   for (unsigned i = 0; i < n; i++) {
      ...
      fprintf(stderr, "cudavk: vsfetch shader %2u launches %8" PRIu64
   ...
   fprintf(stderr, "cudavk: vsfetch census: %u vertex shaders, %" PRIu64
   ...
   fprintf(stderr, "cudavk: vsfetch verdicts:");
   ...
   fputc('\n', stderr);
   if (compared)
      fprintf(stderr, "cudavk: vsfetch launch-weighted regs: classic %.1f "
...
emit_intrinsic(struct ntl_context *ctx, nir_intrinsic_instr *instr)
{
   ...
   default: {
       ...
       * CUDAVK_DEBUG_SHADER, which nobody sets until they already suspect
       * the shader.
       *
...
/* Why a sampler handle could not be matched, for CUDAVK_SPEC_STATS. The
         ...
         nir_foreach_instr(instr, block) {
            ...
            if (!capture_tex_desc_ref(tex, &ref)) {
               ...
               if (cp_debug->spec_stats && !bin->spec_rejected_reported) {
                  ...
                  fprintf(stderr, "cudavk: sampler handle not specialisable: "
                        ...
                        const char *what)
{
   ...
   if (LLVMGetTargetFromTriple(triple, &target, &error) != 0) {
      fprintf(stderr, "cudavk: inline %s target lookup failed: %s\n", what,
              error ? error : "?");
   ...
   if (!ok || !helper_module) {
      fprintf(stderr, "cudavk: inline %s LLVM %d bitcode parse failed\n",
              what, LLVM_VERSION_MAJOR);
      goto out;
   }
   if (strcmp(LLVMGetTarget(helper_module), triple) != 0 ||
       strcmp(LLVMGetDataLayoutStr(helper_module), layout) != 0) {
      fprintf(stderr, "cudavk: inline %s bitcode target/layout mismatch "
              "(LLVM %d)\n", what, LLVM_VERSION_MAJOR);
   ...
   if (LLVMLinkModules2(module, helper_module) != 0) {
      fprintf(stderr, "cudavk: inline %s LLVM module link failed\n", what);
      ok = false;
      goto out;
   }
   ...
   for (unsigned s = 0; s < num_symbols; s++) {
      ...
      if (!helper) {
         fprintf(stderr, "cudavk: inline %s helper %s absent after link\n",
                 what, symbols[s]);
         ok = false;
         goto out;
      }
   ...
   if (pass_error) {
      ...
      fprintf(stderr, "cudavk: inline %s LLVM optimization failed: %s\n",
   ...
   for (unsigned s = 0; s < num_symbols; s++) {
      if (LLVMGetNamedFunction(module, symbols[s])) {
         fprintf(stderr, "cudavk: inline %s helper %s survived forced "
                 "inlining\n", what, symbols[s]);
         ok = false;
         goto out;
      }
...
compile_module_to_ptx(LLVMModuleRef module, int sm_major, int sm_minor, size_t *out_size)
{
   ...
   if (LLVMGetTargetFromTriple(triple, &target, &error) != 0) {
      fprintf(stderr, "cudavk: failed to get NVPTX target: %s\n", error);
      LLVMDisposeMessage(error);
      return NULL;
   }
   ...
   if (!tm) {
      fprintf(stderr, "cudavk: failed to create target machine\n");
      return NULL;
   }
   ...
   if (LLVMTargetMachineEmitToMemoryBuffer(tm, module, LLVMAssemblyFile, &error, &buf) != 0) {
      if (error) {
         fprintf(stderr, "cudavk: PTX emission failed: %s\n", error);
         LLVMDisposeMessage(error);
      }
      LLVMDisposeTargetMachine(tm);
      return NULL;
 ...
 * which the direct one did not before — CUDAVK_MAX_REGISTERS reached the
                   ...
                   int max_regs)
{
   ...
   if (!sampler_ptx && !fs_helper_ptx) {
      CUresult err = cuModuleLoadDataEx(module, shader_ptx, num_jit, jit_opts,
                                       jit_vals);
      if (err != CUDA_SUCCESS)
         fprintf(stderr, "cudavk: CUDA JIT error log:\n%s\ninfo:\n%s\n",
   ...
   if (err != CUDA_SUCCESS)
      fprintf(stderr, "cudavk: CUDA link/JIT error log:\n%s\ninfo:\n%s\n",
              error_log[0] ? error_log : "(empty)",
              info_log[0] ? info_log : "(empty)");
   ...
   if (!cp_debug->no_regcap && !cp_debug->max_registers &&
      ...
      if (cap > 0 && cap < cost.regs) {
         ...
         if (alt_fn && alt_cost.blocks > cost.blocks) {
            ...
            if (cp_debug->shader_stats)
               fprintf(stderr, "cudavk: sampler variant %-7s regs %3d "
 ...
 *
 * This is the compile-time shape of the policy — CUDAVK_REGCAP_STATIC — and
 ...
 *
 * CUDAVK_MAX_REGISTERS overrides everything with a fixed cap on every
 * shader, which is what it always did and is how a regression gets bisected.
 * CUDAVK_REGCAP_STATIC=1 applies the cap at compile time without the trial,
 * which is the shape the plan proposed and is kept so it can be measured.
 * CUDAVK_NO_REGCAP disables all of it.
 */
                         ...
                         const char *stage)
{
   ...
   const char *why = NULL;
   if (forced)
      why = "capped by CUDAVK_MAX_REGISTERS";
   else if (disabled)
      why = "policy off";
   else if (!is_fragment)
   ...
   if (why) {
      if (stats)
         fprintf(stderr, "cudavk: shader %-21s regs %3d spill %4d "
   ...
   if (stats)
      fprintf(stderr, "cudavk: shader %-21s regs %3d spill %4d blocks/sm %d "
                   ...
                   bool hardware_texture, bool vs_fetch)
{
   ...
   if (inline_interp) {
      ...
      if (!input_footprint_safe) {
         if (cp_debug->shader_stats || cp_debug->dump_ir || cp_debug->dump_ptx)
            fprintf(stderr, "cudavk: inline FS declined: input footprint "
   ...
   if (ctx.vs_fetch) {
      ...
      if (!footprint_safe) {
         if (cp_debug->shader_stats || cp_debug->dump_ir || cp_debug->dump_ptx)
            fprintf(stderr, "cudavk: fused vertex fetch declined: an input "
    ...
    * cudavk saw the shader. This is that pass.
    */
   ...
   ctx.module = LLVMModuleCreateWithNameInContext("cudavk_compute", ctx.llvm_ctx);
    ...
    * it does not work here. `CUDAVK_LAUNCH_BOUNDS=N` emits `.maxntid 256`
   ...
   if (LLVMVerifyModule(ctx.module, LLVMReturnStatusAction, &error)) {
      fprintf(stderr, "cudavk: LLVM module verification failed:\n%s\n", error ? error : "(null)");
      LLVMDisposeMessage(error);
      LLVMDisposeBuilder(ctx.builder);
      LLVMDisposeModule(ctx.module);
      LLVMContextDispose(ctx.llvm_ctx);
   ...
   if (ctx.vs_fetch && (strstr(ptx, "cp_vs_fetch_ids") ||
      ...
      fprintf(stderr, "cudavk: fused vertex fetch rejected: the lane helper "
   ...
   if (bad_hardware_ptx ||
      ...
      if (cp_debug->shader_stats || cp_debug->dump_ir || cp_debug->dump_ptx)
         fprintf(stderr, "cudavk: rejected unsafe inline FS PTX "
   ...
   if (cp_debug->dump_ptx)
      fprintf(stderr, "cudavk: generated PTX (%zu bytes):\n%s\n", ptx_size, ptx);
   ...
   if (err != CUDA_SUCCESS) {
      ...
      fprintf(stderr, "cudavk: loading shader module failed (%d: %s)\n",
              err, err_str ? err_str : "?");
                         ...
                         int sm_minor, const char *sampler_ptx)
{
   ...
   if (cp_debug->shader_stats)
      fprintf(stderr, "cudavk: vsfetch verdict %-15s classic regs %3d "
