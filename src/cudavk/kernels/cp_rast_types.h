...
struct cp_rasterize_args {
   ...
   /*
    * TEMPORARY INSTRUMENTATION (CUDAVK_FRAG_CENSUS). Both are uint32 per
   ...
   /*
    * TEMPORARY (CUDAVK_ABUFFER). A counted per-pixel fragment list, built by
 ...
 * to 1 only when CUDAVK_ABUFFER or CUDAVK_FRAG_CENSUS is set, exactly the
 ...
 * which is the failure mode gap 12 in CUDAVK_HANDOFF.md already is.
 */
...
struct cp_fs_interp_args {
   ...
   uint32_t num_samples;
   /*
    * TEMPORARY (CUDAVK_ABUFFER). What this interpolation emitted, folded
   ...
   /*
    * TEMPORARY (CUDAVK_ABUFFER), step 3b. The quad stream as the *source* of
...
/*
 * Composite a whole A-buffer into the colour attachment (CUDAVK_ABUFFER).
 *
 ...
 * that one binary can be A/B'd: CUDAVK_TILE_BOUND=0 in the environment
...
struct cp_vertex_fetch_args {
    ...
    * batching existed — which is what makes CUDAVK_BATCH_MAX=1 a real
