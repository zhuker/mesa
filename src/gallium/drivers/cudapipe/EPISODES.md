...
## Session 17: unsafe device-only overflow assumption

`CUDAPIPE_UNSAFE_NO_OVERFLOW=1` now keeps the episode quad count on the device.
A one-thread prefix kernel converts per-segment counts into compact list ranges,
scatter builds those lists, and fragment shading consumes device-side bases and
counts without the overflow decision readback. The normal path is unchanged.

Merging several segments into one compact shader output exposed a material-row
mapping defect: three Crossroads sentinel frames differed by 32,814--35,978
bytes. Disabling sampler variants did not change the deltas. Keeping one
compact shading group per segment is byte-identical on all three sentinels, so
the flag deliberately uses that correct experimental form until merged compact
rows are fixed.

The launch cost overwhelms the removed readback. The old capture regresses from
about 25.20 ms to 47.57 ms median; Crossroads regresses from about 7.08 ms to
7.75 ms. Artifacts are under
`~/claude-scratchpad/no-overflow/compact-perseg-timing2-1787080383/`, with the
byte-exact probes under
`~/claude-scratchpad/no-overflow/compact-nomerge-1787080312/`. This experiment
is retained only behind the explicitly unsafe flag and is not a candidate for
the default path in its current form.

## Session 18: callable-helper fragment fusion reproduced

The proposed first callable-shader experiment was already present in the
current architecture. Generated fragment `main` is linked with
`cp_fs_helper.ptx` and calls `cp_abuf_interpolate_lane` before executing the
lowered shader body. Thus A-buffer interpolation and fragment shading already
share one generated-shader launch and avoid the intermediate interpolation
launch. This is the useful inverse of making the entire shader body callable:
the generated kernel calls a common device helper, but tests the same launch,
intermediate-memory, derivative-lane, and device-linking questions.

`CUDAPIPE_NO_FUSED_ABUF_INTERP=1` was added as an explicit A/B switch. It
restores `cp_abuf_interpolate[_ranges]` as a separate launch and passes a null
fused-helper argument to generated `main`. Three Crossroads sentinel frames
are byte-identical. The complete 18-sample, 60-frame iteration is
`~/git/Vulkan/build/iter/callable-fs-separate`; against
`buffer-image-device`, every stored frame remains within its frame-zero budget.
The standing NVIDIA differences in `gltfscenerendering` and `texture3d` are
unchanged.

Fresh paired replay measurements confirm that fusion pays. The old capture is
25.22 ms fused versus 26.03 ms separate, a 0.81 ms (3.2%) win. Crossroads is
7.10 ms fused versus 7.22 ms separate, a 0.12 ms (1.7%) win. Raw results are
under `~/claude-scratchpad/perf16/callable-fs-ab-1787082611/`; sentinel output
is under
`~/claude-scratchpad/perf16/callable-fs-separate-sentinels-1787082586/`.

This proves that direct device linking of common fragment helpers, four-lane
derivative-correct execution, and interpolation/shader launch fusion are both
correct and profitable. It does not yet make the lowered shader body a device
function callable by a tile rasterizer. That larger ABI refactor should be
justified by functionality needed for the tile executor, not by repeating the
already-successful interpolation-fusion experiment.
